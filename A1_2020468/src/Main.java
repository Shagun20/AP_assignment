...
public class Main 
{
	...
	public static void main(String[] args) 
	{
		...
		if(n==4) 
		{
			...
			int flag=0;
			Hospital h; //go to the particular hospital's id
		...
		if(n==5)
		{
			...
			if(choice==1) 
			{
				...
				else 
				{
						...
						if(citizens.get(j).getid().equals(citizen_id)) 
							{
								...
								c=citizens.get(j); //c is the citizen here
					...
					int k=1;
					System.out.print("Enter hospital id:"); //assuming valid citizen credentials entered 
					String id=sc.next();
					for(int i=0;i<search_hospitals.size();i++) 
						{
						...
						c.book_slot((h.getslots().get(slot_no)).getvaccine(), (h.getslots().get(slot_no)).getday(),gap, dose_req); //slot no denotes the slot no chosen
					
					    h.getslots().get(slot_no).book_slot();  //set hospital slot as booked for one person
					   
					    System.out.println(c.getname()+" vaccinated with "+ c.get_vaccine()); //gap and ddose are set to a value 0 init.
			...
			else if(choice==2) 
			{
				...
				h=hospitals.get(hospital_ids.get(hospital_id)); //object refernce to hospital
